...
# ------------------------------------------
# Process each rdd from stream
# ------------------------------------------
def process_each(x, percent):
    if x.count() > 0:
        parsed_rdd = x.map(lambda x: parse_tuple(x))
        grouped_rdd = parsed_rdd.groupBy(lambda x: x[0])
        formatted_rdd = grouped_rdd.map(lambda x: parse_reviews_point(x))
        avg_cuisine_reviews = x.count() / formatted_rdd.count()
        vals_removed_rdd = formatted_rdd.filter(lambda x: remove_vals(x, avg_cuisine_reviews, percent))
        sorted_rdd = vals_removed_rdd.sortBy(lambda x: x[1][3], ascending=False)
        return sorted_rdd
...
def my_model(ssc, monitoring_dir, result_dir, percentage_f, window_duration, sliding_duration):
    input_str = ssc.textFileStream(monitoring_dir)
    dictionary_str = input_str.map(lambda x: json.loads(x))
    win_str = dictionary_str.window(window_duration * time_step_interval, sliding_duration * time_step_interval)
    final_str = win_str.transform(lambda x: process_each(x, percentage_f))
    final_str.pprint()
    final_str.saveAsTextFiles(result_dir)
...
if __name__ == '__main__':
    ...
    monitoring_dir = "/FileStore/tables/A02_my_monitoring/"
    checkpoint_dir = "/FileStore/tables/A02_my_checkpoint/"
